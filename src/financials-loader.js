(function(){
  ...
  function fetchProxy(){
    ...
    return fetch(url).then(function(r){
      ...
      // Proxy may return two shapes:
      // 1) { source, data: [ ... ] }
      // 2) { source, data: { data: [...], currency: 'USD' } }
      if(obj && obj.data){
        // shape 2
      ...
      throw new Error('proxy-empty');
          ...
          function ensureFormatted(arr, currency){
            ...
            return arr.map(function(it){
              ...
              // if already contains German comma or percent or unit, leave unless it's transformable
              var s = String(v);
  ...
  function scheduleRefresh(){
    try{
      ...
      // If nextMs is 0, schedule immediate refresh with slight debounce
      var delay = nextMs <= 0 ? 1000 : nextMs;
      __cap_refresh_timer = setTimeout(function(){
        // Attempt refresh but avoid replacing UI with errors — use cache fallback
  ...
  // initial render then schedule next refresh
  ...
  if(!alphaKey){
    ...
    fetchProxy().then(function(d){ renderFallback(d); scheduleRefresh(); }).catch(function(){
      // proxy failed — try cached alpha flow (will use cache if available) then local fallback
