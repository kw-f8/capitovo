...
def generate_hero_svg():
    # Set seed for consistent, "good" looking shape (same as v5/v6/v9/v10/v11/v12/v13/v14)
    random.seed(55) 
    ...
    # Pure opacity fade, sequential. No movement (translate/scale) to avoid "swinging".
    svg_parts.append('<style>')
    # duration 0.5s for a smooth entry
    ...
    for i, candle in enumerate(prices):
        ...
        # Animation delay
        # Slower sequence to simulate "development"
        # 0.06s per candle * 95 candles = ~5.7 seconds total animation
        delay = i * 0.06 
