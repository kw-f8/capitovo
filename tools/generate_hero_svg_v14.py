...
def generate_hero_svg():
    # Set seed for consistent, "good" looking shape (same as v5/v6/v9/v10/v11/v12/v13)
    random.seed(55) 
    ...
    # CSS Animation - IMPROVED V14
    # "Cooler" animation: Slide up + Scale Y + Fade in
    # Using cubic-bezier for a smooth "spring" into place
    svg_parts.append('<style>')
    svg_parts.append('.candle-group { opacity: 0; transform: translateY(20px) scaleY(0.5); transform-origin: center; transform-box: fill-box; animation: appear 0.6s cubic-bezier(0.22, 1, 0.36, 1) forwards; }')
    svg_parts.append('@keyframes appear { from { opacity: 0; transform: translateY(20px) scaleY(0.5); } to { opacity: 1; transform: translateY(0) scaleY(1); } }')
    svg_parts.append('</style>')
    ...
    for i, candle in enumerate(prices):
        ...
        # Animation delay - Slightly faster stagger for "flow" feel
        delay = i * 0.035 
