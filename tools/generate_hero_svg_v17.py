...
def generate_hero_svg():
    # Set seed for consistent, "good" looking shape (same as v5-v16)
    random.seed(55) 
    ...
    # CSS Animation - V17 (Faster)
    svg_parts.append('<style>')
    # Reduced duration from 0.5s to 0.4s for snappier appearance
    svg_parts.append('.candle-group { opacity: 0; animation: appear 0.4s ease-out forwards; }')
    ...
    for i, candle in enumerate(prices):
        ...
        # Animation delay - V17 (Faster)
        # Reduced from 0.06s to 0.04s per candle
        delay = i * 0.04 
