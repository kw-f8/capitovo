...
function relativePrefixToRoot(referenceDirName){
    try{
        ...
        if (idx === -1) return '';
        // Determine if the last segment is a filename (has a dot extension)
        const last = parts[parts.length - 1] || '';
        const isFile = last.indexOf('.') !== -1;
        // number of directory segments after the reference dir
        const dirAfter = parts.length - idx - (isFile ? 2 : 1);
        return '../'.repeat(Math.max(0, dirAfter));
    }catch(e){ return '';}    
}
...
function computeAnalysisLink(a, idx, baseOverride){
    ...
    // If a stored link is `Abonenten/slug.html`, and the current page is
    // inside `Abonenten/` (possibly nested), we must avoid producing
    // `../Abonenten/slug.html` (which leads to Abonenten/Abonenten/...).
    if (/^abonenten\//i.test(raw)){
        const rest = raw.replace(/^abonenten\//i, '');
        const prefixToAb = relativePrefixToRoot('abonenten') || (isInAbonenten ? '../' : '');
        return prefixToAb + rest;
    }
    return base + raw;
}
